...
export default function StudioPage() {
  ...
  const handleGenerate = () => {
    ...
    setTimeout(() => {
      setStatusMessage('Analyzing original audio structure...');
      setTimeout(() => {
        setStatusMessage('Injecting style latents (Stable Audio)...');
        setTimeout(() => {
          setStatusMessage('Synthesizing derivative remix...');
          setTimeout(() => {
            setIsGenerating(false);
            setStatusMessage('Remix generated successfully! Ownership split applied.');
          }, 2000);
        }, 1500);
      }, 1500);
    }, 1000);
    ...
    <div className="min-h-screen bg-[#0B0F19] text-[#F9FAFB] p-8">
      <div className="max-w-4xl mx-auto">
        <header className="mb-8">
          ...
          <p className="text-gray-400 text-sm">Generate derivative tracks legally with automated royalty splits</p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="bg-gray-900 border border-gray-800 p-6 rounded-2xl">
              <h2 className="text-sm uppercase tracking-wider text-gray-500 font-bold mb-4">1. Text Prompt Description</h2>
              <textarea
                rows={3}
            ...
            <div className="bg-gray-900 border border-gray-800 p-6 rounded-2xl">
              <h2 className="text-sm uppercase tracking-wider text-gray-500 font-bold mb-4">2. Choose Genre Style Preset</h2>
              <div className="grid grid-cols-3 gap-3">
                {['lofi', 'dark-phonk', 'synthwave', 'techno', 'anime-opening', 'ambient'].map((preset) => (
                  <button
                    key={preset}
                    onClick={() => setStyle(preset)}
                    className={`py-3 px-4 rounded-xl text-xs font-semibold uppercase tracking-wider border transition ${
                      style === preset ? 'bg-[#8B5CF6] text-white border-transparent' : 'bg-gray-950 border-gray-800 text-gray-400'
                    }`}
                  >
                    {preset.replace('-', ' ')}
                  </button>
            ...
            <button
              ...
              className={`w-full py-4 rounded-2xl font-bold shadow-xl transition flex items-center justify-center space-x-2 ${
                isGenerating ? 'bg-gray-800 text-gray-500 cursor-not-allowed' : 'bg-[#10B981] hover:bg-[#059669] text-black shadow-emerald-500/10'
              }`}
            >
              <span>{isGenerating ? 'GENERATING REMIX...' : 'GENERATE AI REMIX'}</span>
              <span>⚡</span>
            </button>
          ...
          <div className="space-y-6">
            <div className="bg-gray-900 border border-gray-800 p-6 rounded-2xl">
              <h2 className="text-sm uppercase tracking-wider text-gray-500 font-bold mb-4">Royalty Split Invariants</h2>
              ...
              <div className="mt-6 text-xs text-gray-500 leading-relaxed bg-gray-950/60 p-4 rounded-xl border border-gray-800/40">
                ⚠️ **Legal Notice**: This derivative remix will be tracked in the StemVerse Ownership Graph. You cannot sell this remix outside the platform without creator license buyout.
              </div>
            </div>
          </div>
        </div>
