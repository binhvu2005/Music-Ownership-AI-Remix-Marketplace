...
describe('PaymentService', () => {
  ...
  describe('createPaymentIntent', () => {
    ...
    it('should throw an error if license type is disabled', async () => {
      ...
      await expect(
        service.createPaymentIntent('song1', 'commercial', 'user1')
      ).rejects.toThrow('License type commercial is not enabled or available for this song');
    });
    ...
    it('should return a client secret when successful', async () => {
      ...
      const result = await service.createPaymentIntent('song1', 'commercial', 'user1');
  ...
  describe('handleWebhook', () => {
    it('should throw if signature is invalid', async () => {
      ((service as any).stripe.webhooks.constructEvent as jest.Mock).mockImplementation(() => {
        throw new Error('Invalid signature');
      });
      await expect(service.handleWebhook(Buffer.from('body'), 'invalid_sig')).rejects.toThrow('Webhook Error: Invalid signature');
    });
    ...
    it('should insert a license and return true on payment_intent.succeeded', async () => {
      ...
      ((service as any).stripe.webhooks.constructEvent as jest.Mock).mockReturnValue(mockEvent);
      (prisma.userLicense.findUnique as jest.Mock).mockResolvedValue(null); // Not processed yet
      (prisma.userLicense.create as jest.Mock).mockResolvedValue({ id: 'license1' });
    ...
    it('should ignore webhook if already processed (idempotency)', async () => {
      const mockEvent = {
        ...
        data: {
          object: {
            id: 'pi_test123',
            metadata: { songId: 'song1', userId: 'user1', licenseType: 'commercial' },
          },
        },
      };
      ((service as any).stripe.webhooks.constructEvent as jest.Mock).mockReturnValue(mockEvent);
      (prisma.userLicense.findUnique as jest.Mock).mockResolvedValue({ id: 'license1' }); // Already exists
