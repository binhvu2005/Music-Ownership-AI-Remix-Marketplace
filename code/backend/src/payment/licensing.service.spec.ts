...
describe('LicensingService', () => {
  ...
  describe('findMyLicenses', () => {
    it('should return licenses of the user', async () => {
      const mockLicenses = [{ id: 'license1', userId: 'user1', song: { title: 'Song 1' } }];
      (prisma.userLicense.findMany as jest.Mock).mockResolvedValue(mockLicenses);
  ...
  describe('getLicenseOptions', () => {
    it('should return license config if found', async () => {
      ...
      (prisma.songLicenseConfig.findUnique as jest.Mock).mockResolvedValue(mockConfig);
    ...
    it('should throw NotFoundException if config is missing', async () => {
      (prisma.songLicenseConfig.findUnique as jest.Mock).mockResolvedValue(null);
      await expect(service.getLicenseOptions('song1')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('updateLicenseConfig', () => {
    it('should update config if user is the owner', async () => {
      ...
      (prisma.songLicenseConfig.update as jest.Mock).mockResolvedValue({ songId: 'song1', commercialPrice: 1999 });

      const result = await service.updateLicenseConfig('song1', 'owner1', mockUpdateDto);
    ...
    it('should throw ForbiddenException if user is not the owner', async () => {
      ...
      await expect(
        service.updateLicenseConfig('song1', 'owner1', { commercialPrice: 1999 })
  ...
  describe('purchaseFreeLicense', () => {
    it('should throw BadRequestException if license configuration not found', async () => {
      ...
      await expect(service.purchaseFreeLicense('song1', 'user1')).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw BadRequestException if personal license is disabled', async () => {
      ...
      await expect(service.purchaseFreeLicense('song1', 'user1')).rejects.toThrow(
    ...
    it('should throw BadRequestException if user already has this license', async () => {
      ...
      (prisma.userLicense.findFirst as jest.Mock).mockResolvedValue({ id: 'existing_license' });
  ...
  describe('validateLicense', () => {
    it('should return true if active license is found', async () => {
      (prisma.userLicense.findFirst as jest.Mock).mockResolvedValue({ id: 'license1' });
