...
export class LicensingService {
  ...
  async getLicenseOptions(songId: string) {
    ...
    if (!config) {
      throw new NotFoundException(`License configuration for song ${songId} not found`);
    }
    return config;
  }

  async updateLicenseConfig(songId: string, ownerId: string, dto: any) {
    ...
    if (song.ownerId !== ownerId) {
      throw new ForbiddenException(`You do not have permission to update this song's license config`);
    }
  ...
  async purchaseFreeLicense(songId: string, userId: string) {
    ...
    if (!config.personalEnabled) {
      throw new BadRequestException('Personal license is not enabled for this song');
    }
    if (config.personalPrice > 0) {
      throw new BadRequestException('Personal license for this song is not free. Use payment gateway instead.');
    }
    ...
    if (existing) {
      throw new BadRequestException('You already own a Personal license for this song');
    }
