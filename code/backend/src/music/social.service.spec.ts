...
describe('SocialService', () => {
  ...
  describe('toggleLike', () => {
    it('should throw NotFoundException if song does not exist', async () => {
      ...
      await expect(service.toggleLike('invalid-song', 'user-1')).rejects.toThrow(NotFoundException);
    });
    ...
    it('should delete a like if it exists', async () => {
      mockPrismaService.song.findUnique.mockResolvedValue({ id: 'song-1' });
      mockPrismaService.like.findUnique.mockResolvedValue({ userId: 'user-1', songId: 'song-1' });
  ...
  describe('createComment', () => {
    it('should throw BadRequestException if content is empty', async () => {
      await expect(service.createComment('song-1', 'user-1', '   ')).rejects.toThrow(BadRequestException);
    });

    it('should create a comment successfully', async () => {
      mockPrismaService.song.findUnique.mockResolvedValue({ id: 'song-1' });
      const mockComment = { id: 'comment-1', content: 'Nice song!', userId: 'user-1' };
      mockPrismaService.comment.create.mockResolvedValue(mockComment);

      const result = await service.createComment('song-1', 'user-1', 'Nice song!');
  ...
  describe('deleteComment', () => {
    it('should throw ForbiddenException if user is not the author of the comment', async () => {
      mockPrismaService.comment.findUnique.mockResolvedValue({ id: 'comment-1', userId: 'user-different' });
    ...
    it('should delete a comment if user is the author', async () => {
      mockPrismaService.comment.findUnique.mockResolvedValue({ id: 'comment-1', userId: 'user-1' });
