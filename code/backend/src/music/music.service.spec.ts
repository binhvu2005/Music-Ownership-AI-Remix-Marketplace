...
describe('MusicService', () => {
  ...
  describe('update', () => {
    it('should update song successfully if user is owner', async () => {
      const mockSong = { id: 'song-uuid', ownerId: 'owner-uuid', title: 'Original' };
      (prisma.song.findUnique as jest.Mock).mockResolvedValue(mockSong);
      (prisma.song.update as jest.Mock).mockResolvedValue({ ...mockSong, title: 'Updated' });

      const result = await service.update('song-uuid', 'owner-uuid', { title: 'Updated' });
  ...
  describe('remove', () => {
    it('should block deletion if song has active licenses', async () => {
      ...
      await expect(service.remove('song-uuid', 'owner-uuid')).rejects.toThrow(BadRequestException);
    });
    ...
    it('should delete song if no active licenses and user is owner', async () => {
      ...
      expect(prisma.song.delete).toHaveBeenCalledWith({ where: { id: 'song-uuid' } });
    });
  });
});
